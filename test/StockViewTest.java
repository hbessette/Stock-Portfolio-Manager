...
public class StockViewTest {
  ...
  public void testStockView() {
    ...
    assertEquals("sell-all-stock (to sell all of a stock in specific portfolio)", viewLog[13]);
    assertEquals("evaluate-portfolio (to evaluate total value of a specific portfolio)", viewLog[14]);
    assertEquals("get-composition (to get the composition of stocks of a specific portfolio)", viewLog[15]);
    assertEquals("get-distribution (to get the distribution of stocks of a specific portfolio)", viewLog[16]);
    assertEquals("performance-over-time (to get a graph of a portfolio's stock)", viewLog[17]);
    assertEquals("get-price-change (to get the price change of a stock from and to specific dates)", viewLog[18]);
    assertEquals("get-x-day-crossovers (to get the x day crossovers of a stock)", viewLog[19]);
    assertEquals("get-x-day-average (to get the average of a stock over a specific period)", viewLog[20]);
